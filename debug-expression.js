...
function debugExpressionManager() {
  ...
  if (internalModel.expressionManager) {
    ...
    if (Array.isArray(exprMgr.definitions)) {
      console.log("Definitions is an array with", exprMgr.definitions.length, "items");
      exprMgr.definitions.forEach((expr, index) => {
        console.log(`Expression ${index}:`, expr);
      });
    } else if (exprMgr.definitions) {
      console.log("Definitions is not an array, keys:", Object.keys(exprMgr.definitions));
...
function testExpressions() {
  ...
  if (internalModel.expressionManager?.definitions) {
    ...
    console.log("Found expressions via expressionManager.definitions:", expressions.length);
  }
  ...
  // Method 2: settings.expressions (Cubism 2.1)
    ...
    console.log("Found expressions via settings.expressions:", expressions.length);
  }
...
console.log("Debug functions loaded. Run debugExpressionManager() or testExpressions() after loading a model.");
